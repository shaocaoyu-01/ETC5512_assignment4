...
# 2. 将面板数据和空间数据合并，得到每个省每年的几何与指标
map_data <- panel_all %>%
  # 确保 Year 是数值型
  mutate(Year = as.integer(Year)) %>%
  # 内连接保证只留下两边都有的省份+年份组合
  left_join(china_sf, by = c("Region" = "NAME_1")) %>%
  st_as_sf()

# 3. 搭建 ggplot 对象（不含动画）
p <- ggplot(map_data) +
  geom_sf(aes(fill = GDP_growth_rate), color = "grey90", size = 0.2) +
...
anim <- animate(
  p_anim,
  nframes = length(unique(map_data$Year)) * 2,  # 每年两帧，更平滑
  fps     = 5,                                  # 每秒 5 帧
  width   = 800,
  height  = 600,
  renderer = gifski_renderer()                  # GIF
)

anim_save("china_gdp_growth.gif", animation = anim)
